...
> **作者**: 叶东成
> **日期**: 2026-04-07
> **配套文档**: [产品手册](PRODUCT_MANUAL_zh.md)
...
### 2.1 `tritonkit.testing` — 统一正确性测试

Triton kernel 的统一正确性测试框架。
...
#### `run_on_available_backends` — 跨硬件验证
...
### 2.2 `tritonkit.bench` — 统一 Benchmark 框架

硬件感知的统一 benchmark 框架。
...
### 2.3 `tritonkit.primitives` — 可组合原语
...
### 2.4 `tritonkit.integrate` — 集成工具

将 Triton kernel 连接到 PyTorch 生态的工具。
...
### 2.5 `tritonkit.examples` — 示例 Kernel

使用原语构建的参考 kernel。每个示例包含：
- 一个 `@triton.jit` kernel 函数
- 一个可从 PyTorch 调用的 Python 包装器
- `tests/test_examples.py` 中的对应测试
- `benchmarks/suites/` 中的对应 benchmark
...
其他示例遵循相同模式。完整实现位于 `src/tritonkit/examples/`。

---

### 2.6 `tritonkit` CLI — 命令行工具

入口点通过 `pyproject.toml` 注册：
...
- 所有测试和 benchmark **需要本地 GPU**
- `pytest tests/` 运行正确性测试
- `tritonkit bench --suite=all --format=json --output=results/ci.json` 运行 benchmark
- GitHub Actions：使用搭载 RTX 3080 Ti 的自托管 runner
- 后续：使用 vast.ai runner 覆盖 H100/MI300
...
| 静态检查 | `ruff` 默认规则 |
| 类型检查 | `mypy --strict`（非 Triton 代码） |
| 文档字符串 | Google 风格 |
| Triton kernel | `@triton.jit`，以 `_` 前缀（私有），constexpr 参数用 camelCase |
| Python 包装器 | snake_case，完整类型注解 |
| 测试 | pytest，每个模块一个测试文件 |
| 导入 | 公开 API 使用 `import tritonkit as tk` |

---

## 7. 实现优先级

Phase 1 实现顺序（第 1-6 周）：

| 顺序 | 文件 | 为什么先做 |
|------|------|-----------|
| 1 | `pyproject.toml` | 项目骨架，pip install 可用 |
| 2 | `src/tritonkit/__init__.py`, `_version.py` | 包存在 |
| 3 | `src/tritonkit/testing/shapes.py` | 零依赖，即刻可用 |
| 4 | `src/tritonkit/testing/correctness.py` | 核心功能 #1 |
| 5 | `src/tritonkit/bench/hardware.py` | benchmark 框架需要 |
| 6 | `src/tritonkit/bench/roofline.py` | 峰值测量需要 |
| 7 | `src/tritonkit/bench/result.py` | 结果数据模型 |
| 8 | `src/tritonkit/bench/runner.py` | 核心功能 #2 |
| 9 | `src/tritonkit/bench/baselines.py` | 内置对比基线 |
| 10 | `src/tritonkit/primitives/*.py` | 构建块 |
| 11 | `src/tritonkit/examples/rmsnorm_fused.py` | 首个展示 kernel |
| 12 | `src/tritonkit/examples/gemm_fp16.py` | GEMM 展示 |
| 13 | `src/tritonkit/examples/flash_attention.py` | Attention 展示 |
| 14 | `src/tritonkit/cli/main.py` | CLI 界面 |
| 15 | `tests/*` | 验证一切 |
| 16 | `benchmarks/suites/*` | 产出首批 benchmark 数据 |
